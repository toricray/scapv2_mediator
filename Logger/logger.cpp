...
int Logger::openNewFile(LogLevel lvl)
{
...
{
    //Заполняем пул сообщений
    // пул нужен, чтобы не создавать сообщения каждый раз
    // сообщения нужны, чтобы писать логи асинхронно, не тормозя основные потоки записью на диск
    if(!instance)
    {
        instance = new Logger();
    ...
    int ret = mkdir(instance->logPath.c_str(), 0777);
    if(ret != 0 && errno != EEXIST)
        return -1;

    //Создаем файлы для каждого уровня логирования, если нужно
    if(instance->logInSeparateFiles)
    {
        for(int i = 0; i < MAX_LOGLVL; i++)
        {
            ...
            else
            {
                ...
                if(instance->openNewFile((LogLevel) i) == -1)
                    return -1;
            }
        }
    }
    ...
    //Инициируем запись в сислог, если нужно
	if(instance->logToSyslog)
	{
    ...
    //Запускаем обрабатывающий поток
...
LogMessage& Logger::getLogMsg(LogLevel lvl)
{
    ...
    if(instance)
    {
        ...
        {
            //svc не успевает записывать данные
            //текущее сообщение в итоге не будет записано
            //это плохо, но лучше, чем задержка основных потоков
        }
    }
    return *msg;
}
